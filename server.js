...
const ID_SALT = process.env.ID_SALT || "salt-secret-12345";

function encryptId(text) {
    const cipher = crypto.createCipheriv('aes-256-ctr', crypto.scryptSync(ID_SALT, 'salt', 32), Buffer.alloc(16, 0));
    return Buffer.concat([cipher.update(Buffer.from(text)), cipher.final()]).toString('base64url');
}

function decryptId(hash) {
    try {
        const decipher = crypto.createDecipheriv('aes-256-ctr', crypto.scryptSync(ID_SALT, 'salt', 32), Buffer.alloc(16, 0));
        return Buffer.concat([decipher.update(Buffer.from(hash, 'base64url')), decipher.final()]).toString();
    } catch (e) {
        return null;
    }
}
...
async function findSession(sessionId) {
    const decryptedId = decryptId(sessionId) || sessionId;
    const primaryShard = getShardIndex(decryptedId);
    try {
        const col = await getDatabase(primaryShard);
        const sessionData = await col.findOne({ 
            $or: [
                { session_id: decryptedId },
                { public_token: sessionId }
            ]
        });
    ...
    for (let i = 0; i << DB DB_SHARDS.length; i++) {
...
app.get('/go/:sessionId', rateLimiter, async (req, res) => {
    ...
    if (!isValidSessionId(sessionId)) {
        return res.status(400).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Invalid</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>❌ Invalid Link</h1></div></body></html>');
    }
    ...
    try {
        ...
        if (!result) {
            ...
            return res.status(404).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Not Found</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>🔍 Link Not Found</h1></div></body></html>');
        }
        ...
        if (ageSeconds > 900) {
            return res.status(410).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Expired</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>⏰ Link Expired</h1></div></body></html>');
        }
        if (sessionData.used) {
            return res.status(410).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Used</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>✅ Already Used</h1></div></body></html>');
        }
        if (sessionData.access_count >= 3) {
            return res.status(403).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Blocked</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>🚫 Too Many Attempts</h1></div></body></html>');
        }
        if (!sessionData.short_url || !isValidUrl(sessionData.short_url)) {
            return res.status(400).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Invalid</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>⚠️ Invalid Destination</h1></div></body></html>');
        }

        const minWaitMs = 3000;
        const timeSinceCreation = Date.now() - new Date(sessionData.created_at).getTime();
        if (timeSinceCreation << min minWaitMs) {
            await new Promise(resolve => setTimeout(resolve, minWaitMs - timeSinceCreation));
        }

        await collection.updateOne({ 
            $or: [{ session_id: sessionData.session_id }, { public_token: sessionId }] 
        }, { $inc: { access_count: 1 } });
...
<<metameta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"> 
<<metameta name="referrer" content="no-referrer"> 
<<metameta name="theme-color" content="#1a1a2e"> 
<<titletitle>🔐 Click to Continue</title> 
<<stylestyle>${SHARED_STYLES}</style> 
<<svgsvg style="position:absolute;width:0;height:0"><<defsdefs><<linearlinearGradient id="shieldGradient" x1="0%" y1="0%" x2="100%" y2="100%"><<stopstop offset="0%" style="stop-color:#667eea;stop-opacity:1"/><<stopstop offset="50%" style="stop-color:#764ba2;stop-opacity:1"/><<stopstop offset="100%" style="stop-color:#667eea;stop-opacity:1"/></linearGradient></defs></svg> 
</head> 
<<bodybody> 
<<divdiv class="container" id="mainContainer"> 
<<divdiv class="content"> 
<<divdiv class="icon-container"> 
<<divdiv class="shield-wrapper" id="shieldWrapper"><<divdiv class="shield-icon"><<svgsvg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><<pathpath class="shield-body" d="M50 5 L20 18 V45 C20 65 35 85 50 92 C65 85 80 65 80 45 V18 L50 5 Z"/><<pathpath class="shield-check" d="M35 50 L48 63 L65 40"/></svg></div></div> 
<<divdiv class="loader-wrapper" id="loaderWrapper"><<divdiv class="loader"></div></div> 
<<divdiv class="check-mark" id="checkMark">✓</div> 
<<divdiv class="cross-mark" id="crossMark">✗</div> 
</div> 
<<hh2 id="title">🚀 Human Verification</h2> 
<<pp class="message" id="message">Click the button below to continue</p> 
<<buttonbutton id="clickVerifyBtn" class="click-verify-btn">👆 Click to Continue</button> 
<<divdiv class="countdown-box" id="countdownBox"><<spanspan id="countdown">3</span> seconds remaining</div> 
<<divdiv class="progress-bar" id="progressBar" style="display:none;"><<divdiv class="progress" id="progress"></div></div> 
<<pp class="status-message" id="statusMessage">Waiting for click...</p> 
<<divdiv class="security-badge"><<svgsvg class="lock-icon" viewBox="0 0 24 24"><<pathpath d="M12 2C9.243 2 7 4.243 7 7V10H6C4.897 10 4 10.897 4 12V20C4 21.103 4.897 22 6 22H18C19.103 22 20 21.103 20 20V12C20 10.897 19.103 10 18 10H17V7C17 4.243 14.757 2 12 2ZM12 4C13.654 4 15 5.346 15 7V10H9V7C9 5.346 10.346 4 12 4ZM12 14C13.103 14 14 14.897 14 16C14 17.103 13.103 18 12 18C10.897 18 10 17.103 10 16C10 14.897 10.897 14 12 14Z"/></svg><<spanspan>Secure Redirect</span></div> 
</div> 
</div> 
<<scriptscript>${ANTI_BYPASS_JS}</script> 
<<scriptscript> 
(function(){ 
'use strict'; 
...
btn.addEventListener('click', startVerification, { passive: true }); 
    ...
    return res.status(500).send('<!DOCTYPE html><<htmlhtml><<headhead><<metameta charset="UTF-8"><<titletitle>Error</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>⚠️ Server Error</h1></div></body></html>'); 
} 
});
...
app.get('/link/:sessionId', rateLimiter, async (req, res) => {
    ...
    if (!isValidSessionId(sessionId)) {
        return res.status(400).send('<!DOCTYPE html><<htmlhtml lang="en"><<headhead><<metameta charset="UTF-8"><<titletitle>Invalid</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>❌ Invalid Link</h1></div></body></html>');
    }

    try {
        const result = await findSession(sessionId);
        if (!result) {
            return res.status(404).send('<!DOCTYPE html><<htmlhtml lang="en"><<headhead><<metameta charset="UTF-8"><<titletitle>Not Found</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>🔍 Link Not Found</h1></div></body></html>');
        }

        const { sessionData, collection } = result;
        const ageSeconds = Math.floor((Date.now() - new Date(sessionData.created_at).getTime()) / 1000);
        if (ageSeconds > 900) {
            return res.status(410).send('<!DOCTYPE html><<htmlhtml lang="en"><<headhead><<metameta charset="UTF-8"><<titletitle>Expired</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>⏰ Link Expired</h1></div></body></html>');
        }
        if (sessionData.used) {
            return res.status(410).send('<!DOCTYPE html><<htmlhtml lang="en"><<headhead><<metameta charset="UTF-8"><<titletitle>Used</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>✅ Already Used</h1></div></body></html>');
        }
...
<<divdiv class="content"> 
<<divdiv class="icon-container"> 
<<divdiv class="shield-wrapper"><<divdiv class="shield-icon"><<svgsvg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><<pathpath class="shield-body" d="M50 5 L20 18 V45 C20 65 35 85 50 92 C65 85 80 65 80 45 V18 L50 5 Z"/><<pathpath class="shield-check" d="M35 50 L48 63 L65 40"/></svg></div></div> 
<<divdiv class="loader-wrapper"><<divdiv class="loader"></div></div> 
</div> 
<h2>🔄 Redirecting</h2> 
<<pp class="message">Loading your destination...</p> 
<<divdiv class="progress-bar"><<divdiv class="progress" id="progress"></div></div> 
<<pp class="status-message" id="statusMessage">Preparing redirect...</p> 
...
window.location.replace(dest); 
        ...
        return res.status(500).send('<!DOCTYPE html><<htmlhtml lang="en"><<headhead><<metameta charset="UTF-8"><<titletitle>Error</title></head><<bodybody style="background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:sans-serif;text-align:center;padding:20px;"><<divdiv><<hh1>⚠️ Server Error</h1></div></body></html>'); 
    } 
});
...
app.post('/api/store-session', async (req, res) => {
    ...
    try {
        ...
        const obfuscatedId = encryptId(session_id);
        const shardIndex = getShardIndex(session_id);
        const collection = await getDatabase(shardIndex);
        await collection.updateOne(
            { session_id: session_id },
        ...
        console.log(`[Store] ✅ Stored: ${session_id} -> Token: ${publicToken}`);
        return res.json({ success: true, public_token: publicToken });
    } catch (error) {

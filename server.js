...
if (process.env.VERCEL !== '1') {
    setInterval(() => {
        ...
        // Cleanup unbounded Maps
        for (const [key, data] of behaviorTracking) {
            if (data.requestTimes.length === 0 || (now - data.requestTimes[data.requestTimes.length - 1]) > 120000) {
                behaviorTracking.delete(key);
            }
        }
        for (const [ip, data] of ipBehaviorMap) {
            if (data.blocked && now >= data.blockUntil) {
                ipBehaviorMap.delete(ip);
            } else if (!data.blocked && data.requests.length === 0) {
                ipBehaviorMap.delete(ip);
            }
        }
...
app.get('/access/:sessionId', detectHeadlessBrowser, detectBrowserExtension, trackIPBehavior, rateLimiter, async (req, res) => {
    ...
    try {
        ...
        // Show a transition page with visual feedback similar to go page
...
<h2 id="title">🔗 Processing Your Link</h2>
<p class="message" id="message">Please wait while we validate your session...</p>
...
function showSuccess(msg) {
    ...
    if(title) title.textContent = '✅ Session Valid';
    if(message) message.textContent = msg || 'Redirecting...';
    if(statusMessage) {
        statusMessage.textContent = 'Opening verification...';
...
function showError(msg) {
    ...
    if(title) title.textContent = '❌ Session Error';
...
function init() {
    ...
    updateStatus('🔗 Validating your session...', '#a5b4fc');
    if(message) message.textContent = 'Please wait while we process your link';
    
    // Simulate validation then redirect
    setTimeout(() => {
        showSuccess('Session validated!');
        setTimeout(() => handleRedirect('/go/' + sessionId), 1200);
    }, 1800);
}
...
app.post('/api/verify-click', rateLimiter, async (req, res) => {
    const { session_id, click_time, mouse_movements } = req.body;
    ...
    try {
        ...
        console.log(`[Verify-Click] Session: ${session_id} | Clicks: ${click_time} | Mouse: ${mouse_movements}`);
        return res.json({ success: true, message: 'Click verified' });
    } catch (error) {
